...
class ClusterTreeBuilder(TreeBuilder):
    ...
    def process_cluster(
        self, cluster, new_level_nodes, next_node_index, summarization_length, lock
    ):
    ...
    def construct_tree(
        ...
        logging.info(f"Num of layers: {self.num_layers}")
        for layer in range(self.num_layers):
            ...
            logging.info(
                f"======Constructing Layer {layer}, next_node_index:{next_node_index}, num of nodes of current layer:{len(node_list_current_layer)}======"
            )

            # @TODO It need extraordinary computation resources to complete
            ...
            # Set up the preliminary nodes for the next layer
            layer_to_nodes[layer + 1] = list(new_level_nodes.values())
            # Change the nodes of the current layer to the new nodes
